...
class OrderController {
    ...
    sendEmail(req, res, next) {
        ...
        content += `
            <div>
                ...
                <p>Bạn đã đặt một đơn hàng mới từ  <a style="color: #FF00E2; text-decoration: none;" href="http://localhost:3000/">Azey_Shop</a> !</p>
    ...
    show(req, res, next) {
        ...
        if (req.body.deli) {
            option.delivered = req.body.deli;
        }

        Promise.all([
            orderModel.find(option, null, { sort: { createdAt: -1 } }),
        ...
        try {
            ...
            if (order) {
                ...
                if (order.status === 'Completed') {
                    var date = new Date();
                    order.deliveredAt = date.setDate(date.getDate() + 3);
                }
            }
            orderModel
    ...
    delivered = async (req, res, next) => {
        try {
            ...
            if (order) {
                ...
                if (req.body.saveDelivery === 'Delivering') {
                    var date = new Date();
                    order.deliveredAt = date.setDate(date.getDate() + 2);
                } else if (req.body.saveDelivery === 'Delivered') {
